...
var sendgrid = __toESM(require_mail3());
var handler = async (event) => {
  ...
  const isSuccessfulResponse = resp[0].statusCode >= 200 || resp[0].statusCode <= 299;
  return {
    statusCode: resp[0].statusCode,
    body: JSON.stringify({
      message: isSuccessfulResponse ? "Successfully sent message" : "Error sending message",
      details: resp[0].body
    })
  };
};
